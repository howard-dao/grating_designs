function [] = f_design_AIM_gc_duallayer(filepath, fill_top_override, ...
                                        fill_bot_override, ...
                                        fill_vs_top_bot_both, geometry, ...
                                        MFD, apodized, run_fdtd, fdtd_opts)
...
%   geometry
%       default aim layer stack or custom. either 'shallow' or 'full'
%   MFD
%       units in nm
%   apodized
...
filename = 'synth_obj_aimgc.mat';

% input wg options
input_wg_type       = 'none';       % for synthesizing object
wg_interface_type   = 'silicon';    % for fdtd

% load synth_obj
synthdata = load( [ filepath filesep filename ] );
synth_obj = synthdata.synth_obj;

% function to enforce minimum feature sizes
enforce_min_feat_size_func = @( period, top_fill, bot_fill, offset) ...
   f_enforce_min_feat_size_aim( period, top_fill, bot_fill, offset, geometry);
...
if run_fdtd
    ...
    % run fdtd
    tic;
    [ fdtd_results ] = f_run_fdtd_2Dgrating_aim(synth_obj, geometry, ...
                                                'dual', wg_interface_type, ...
                                                [ save_data_path filesep '2dfdtd.fsp' ], ...
                                                fdtd_opts );
    toc;
